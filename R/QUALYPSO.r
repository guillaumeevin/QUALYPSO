...
#' get.Qstar.mat
#'
#' Provide matrix containing Helmert contrasts (see Eq. A10 in Tingley et al., 2011) 
#'   Ex |  0  0 -3|
#    |  0 -2  1|
#    | -1  1  1|
#    |  1  1  1|
#' 
#' @references Tingley, Martin P. 2011. A Bayesian ANOVA Scheme for Calculating Climate Anomalies,
#' with Applications to the Instrumental Temperature Record. Journal of Climate 25 (2). https://doi.org/10.1175/JCLI-D-11-00008.1.
#'
#' @param p integer
#'
...
#' get.Qmat
#'
#' Provide matrix Q derived from a matrix of Helmert contrasts (see Eq. A13 in Tingley et al., 2011)
#'
#' @return \item{matrix}{Q}
#'
#' @references Tingley, Martin P. 2011. A Bayesian ANOVA Scheme for Calculating Climate Anomalies,
#' with Applications to the Instrumental Temperature Record. Journal of Climate 25 (2). https://doi.org/10.1175/JCLI-D-11-00008.1.
#'
...
get.Qmat = function(p){
  ...
  # get Q: Eq. A13 in Tingley et al. (2011) "A Bayesian ANOVA scheme..."
...
#'
#' fit trends for each simulation chain of an ensemble of nS projections. Each projection can be a time series
#' of nY time steps (e.g. number of years)
#'
#' @param Y matrix ofsimulation chains: nS x nY  
...
#' QUALYPSO.ANOVA.i
#'
#' Partition sources of uncertainty in climate change responses for one lead time or grid point 'i'
#'
#' @param phiStar.i vector of climate change response at time t: nS x 1
#' @param nMCMC number of MCMC simulation
#' @param listEff # list of effects, each element being a vector of main effect for one type of effect, ex. \code{list(c('GCM1','GCM2'),c('RCM1','RCM2'))}
#' @param scenAvail # matrix of available combinations, ex \code{cbind(c('GCM1','GCM1','GCM2'),c('RCM1','RCM2','RCM1'))}
#' @param scenComp # matrix of possible combinations, ex \code{cbind(c('GCM1','GCM1','GCM2','GCM2'),c('RCM1','RCM2','RCM1','RCM2'))}
#'
#' @return  list with the following fields:
#' \itemize{
...
#' }
#' @examples
#' QUALYPSO.i(phiStar.i=c(0.1,0.5,1.3),nMCMC=1000,listEff=list(c('GCM1','GCM2'),c('RCM1','RCM2')),
#' scenAvail=cbind(c('GCM1','GCM1','GCM2'),c('RCM1','RCM2','RCM1')),
#' scenComp=cbind(c('GCM1','GCM1','GCM2','GCM2'),c('RCM1','RCM2','RCM1','RCM2')))
#'
...
#'   \item \strong{computeEmpEff}: in Development 
#' }
#'
#' @return  list with the following fields:
#' \itemize{
...
#' }
#' @examples
#' QUALYPSO(phiStar=cbind(c(0.1,0.5,1.3),c(1.1,1.5,2.3),c(2.1,2.5,3.3)),
#' scenAvail=cbind(c('GCM1','GCM1','GCM2'),c('RCM1','RCM2','RCM1')))
#' 
#' @references Evin, G., B. Hingray, J. Blanchet, N. Eckert, S. Morin, and D. Verfaillie.
#' Partitioning Uncertainty Components of an Incomplete Ensemble of Climate Projections Using Data Augmentation.
...
#' @param listOption (optional) list of options
#' \itemize{
#'   \item \strong{vecYears}: vector of years corresponding to the projections
#'   \item \strong{parSmooth}: smoothing parameter \code{spar} in \link[stats]{smooth.spline}: varies in [0,1]
#'   \item \strong{indexReferenceYear}: index corresponding to the control year (e.g. 1990)
...
QUALYPSO = function(Y,scenAvail,vecYears=NULL,indexReferenceYear=NULL,futureYear=NULL,listOption=NULL){
  ...
  ANOVAVARIANCE$TotalVar = colsums(anova$varEffect) + anova$varResidualEffect + varInterVariability
